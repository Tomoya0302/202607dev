...
- **統合/枝刈り**：他EMSに完全内包されるEMSを除去（maximal性の維持）。最小寸法が残手荷物の最小寸法未満のEMSを除去。候補は「高さ（z1）」でソートし固定長N（初期値80、GOPT準拠）に padding/clip。
  EMS予算は各コンテナ80（`packing_core/constants.py` の `StageParams.ems_top_n_per_container=80`。
  実装契約の詳細は `実装詳細仕様書.md` §3.5/§4.4 を参照）。

#### 2.3 既配置集合・複数コンテナ状態・シリアライズ
- **既配置集合**：各アイテムを `{id, 種類, 優先flag, コンテナidx, FLB座標(x,y,z), 寸法(l,w,h), 回転(rx,ry,rz), 重量}` で保持。
  ただし runtime observation からは種類（kind）を取得できないため、提出時推論は kind に依存せず、
  runtime 特徴は `is_soft`/`is_priority` を用いる（詳細は `実装詳細仕様書.md` §3.3/§4.4）。
...
- **手荷物特徴**：2×3行列 `[[l,w,h],[w,l,h]]`（0°/90°姿勢）＝Item Representation(IR)。加えて種類one-hot・優先flag・正規化重量を連結。
  ただし runtime observation では種類（kind）を取得できないため、提出時推論は kind に依存せず、
  runtime 特徴は `is_soft`/`is_priority` を用いる（詳細は `実装詳細仕様書.md` §3.3/§4.4）。
...
| 日付 | 内容 |
| --- | --- |
| 2026-07-18 | T-012 契約整合：runtime observation から種類（kind）を取得できないことを踏まえ、提出時推論は kind に依存せず runtime 特徴は `is_soft`/`is_priority` を用いる旨を §2.3・§6 に注記。EMS予算（各コンテナ80）が `packing_core/constants.py` の `StageParams.ems_top_n_per_container` と対応することを §2.2 に注記。詳細は `実装詳細仕様書.md` §3.3/§3.5/§4.4、根拠は `interface_notes.md` §H を参照。本節を新設 |
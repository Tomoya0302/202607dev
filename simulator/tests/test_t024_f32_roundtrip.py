...
# check_inclusionのmarginは6面（Z底面含む）へ一様に適用されるため（contains_oriented_box→
# aabb_contains、実装済み・T-014確定）、コンテナの文字通りの床(INNER_MIN_REL[2])へEMSを
# フラッシュさせるとX/Y境界に関わらずZでINCLUSION不合格になる。X/Y境界検証が主目的の
# fixtureでは、Zを床から十分離した安全値を使う（Z自体の境界検証はBND-ZBOT/BND-CEILが担当）。
...
def _fixture_bnd_zbot():
    """Z支持面フラッシュ（slack非加算）のsanity往復検証。"""
    item = _item()
    ems = EMSBox(
...
def _fixture_bnd_ceil():
    ...
    CEILINGはcandidate_generation_slackの対象外（§4.12「Z方向の契約」）のため、厳密な
    ゼロ余裕の等号境界ではなく、必要余裕(0.023)へごくわずかな実余裕（1e-6、X/Yのslackと
    ...
    tiny_buffer = 1e-6
    # box_top = ems.min_z+osize[2] とし、
    # box_top + required_clearance == inner_max_rel[2] - tiny_buffer となるよう ems.min_z を選ぶ
    # （ちょうど境界より tiny_buffer だけ内側＝実余裕を持たせる）。
    ems_min_z = float(INNER_MAX_REL[2]) - OSIZE[2] - required_clearance - tiny_buffer
    # ems.max_rel[2] は候補のZ位置（ems.min_rel[2]由来）に影響しないため、DIMS事前チェック
    # （ems.size()[2]>=osize[2]）が浮動小数点の減算誤差で偶発的に不合格化しないよう、
    # ems側のZサイズにだけ十分な余裕（1e-4）を追加する。
    ems = EMSBox(
        min_rel=np.array([-0.3, -0.3, ems_min_z], dtype=np.float64),
        max_rel=np.array([0.3, 0.3, ems_min_z + OSIZE[2] + 1e-4], dtype=np.float64),
    )
    return item, ems
...
def _fixture_bnd_overlap():
    ...
    # アンカー式（§4.12）を手計算で適用: box_min_x = ems.min_x + clr、box_max_x = box_min_x+osize[0]。
...
# --- F32-002: Z支持面側はslack非加算であることが往復後も維持される -----------------------


def test_f32_002_z_bottom_has_no_slack_after_roundtrip():
    item, ems = _fixture_bnd_zbot()
    state = _state_for([ems])
    cand, rt_cand = _generate_and_roundtrip(state, item, ems, PP0)

    # 元候補のZはslack非加算（ems.min_z + osize/2 と厳密一致）。
    assert cand.pos_rel[2] == pytest.approx(float(ems.min_rel[2]) + cand.osize[2] / 2.0)
    # 往復後もfloat32丸め誤差の範囲でしか変化しない（XYのslack量1e-6より十分小さいずれのみ許容）。
    assert abs(float(rt_cand.pos_rel[2]) - float(cand.pos_rel[2])) < 1e-5


# --- F32-003: 無関係方向（Z支持面・天井・L_PATH）へ余裕を加えない --------------------------


def test_f32_003_unrelated_directions_receive_no_extra_clearance():
    """Z支持面側はcandidate_generation_slackを加算しない。ceiling_marginは既存判定へ
    委譲し候補生成時に新規余裕を追加しない（生成直後の pos_rel を直接検証）。"""
    from src.packing_core.candidates import candidate_from_ems

    item, ems = _fixture_bnd_zbot()
    cand = candidate_from_ems(item, container_idx=0, orientation=0, ems_id=0, ems=ems, pp=PP0)
    assert cand is not None
    # Z: ems.min_rel[2]+osize[2]/2 と厳密一致（slack非加算）。
    assert cand.pos_rel[2] == pytest.approx(float(ems.min_rel[2]) + cand.osize[2] / 2.0)

    # slackを大きく変えてもZ位置が変化しないことを重ねて確認（無関係方向への非依存）。
    ...
    assert cand_big_slack.pos_rel[2] == pytest.approx(cand.pos_rel[2])

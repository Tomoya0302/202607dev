"""`drift_diag.py` を複数タスクへ並列適用し、SKU別傾向・validator NG との相関を
集計する（findings §17.4 (b)(c) の検証）。"""
...
def _run_one(config_path: str, module_path: str, env_overrides: dict) -> dict:
    ...
    proc = subprocess.run(
        [sys.executable, "-m", "scripts.drift_diag",
...
def main() -> None:
    ...
    print(f"running drift_diag on {len(task_paths)} tasks, extra_env={env_overrides}, "

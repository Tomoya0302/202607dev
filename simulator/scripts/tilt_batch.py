"""`tilt_diag.py` を複数タスクへ並列適用する（`drift_batch.py` と同じ設計）。

ソフト荷物起因の転倒仮説（docs/findings_2026-07.md 新設予定 §25）の検証用。
"""
...
def _run_one(config_path: str, module_path: str, hard_soft: bool, env_overrides: dict) -> dict:
    env = dict(os.environ)
    env.update(env_overrides)
    cmd = [sys.executable, "-m", "scripts.tilt_diag",
           "--config-path", config_path, "--module-path", module_path]
    if hard_soft:
        cmd.append("--hard-soft")
    proc = subprocess.run(
        cmd,
...
def main() -> None:
    ...
    parser.add_argument("--hard-soft", action="store_true",
                        help="soft品の接触コンプライアンスのみ無効化するアブレーションを全タスクへ適用")
    ...
    print(f"running tilt_diag on {len(task_paths)} tasks, hard_soft={args.hard_soft}, "
          f"extra_env={env_overrides}, workers={args.workers}", file=sys.stderr)
    os.makedirs(os.path.dirname(args.out_path) or ".", exist_ok=True)
    n_done = 0
    with open(args.out_path, "w") as out_f, ProcessPoolExecutor(max_workers=args.workers) as pool:
        futures = {
            pool.submit(_run_one, p, args.module_path, args.hard_soft, env_overrides): p
            for p in task_paths
        }
